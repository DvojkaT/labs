...
class datetime
{
	...
	void print_information_prev_year(datetime*, int);
};
...
int main()
{
	...
	cout << "даты которым предшествует другой год" << endl;
	for (int i = 0; i < count_lines; i++)
	{
		if (check_prev_month(dates[i].month, dates[i].day))
			dates[i].print_information_prev_year(&dates[i], i + 1);
	}
...
void datetime::print_information_prev_year(datetime *date, int n)
{
	cout << "№ " << n << "; дата: " << date->day << "." << date->month << "." << date->year << ";\t";
	date->previous_date(date);
...
bool check_prev_month(int month,int day)
{
	if (((month-1) < 1)&&(day==1)) return true;
		return false;
}
